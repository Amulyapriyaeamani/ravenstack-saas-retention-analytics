...
Source: Kaggle — River @ Rivalytics (https://www.kaggle.com/datasets/rivalytics/saas-subscription-and-churn-analytics-dataset)
...
Step	Files	Views Created	Analytical Focus
...
27	`cohort_retention_analysis.sql`	3 views	Month 1 deep dive, 2023 vs 2024, reactivation gap
...
December 2024: 20.17% — 96 accounts in a single month
...
Usage does NOT predict churn — 13 of 13 usage-related hypotheses refuted or partially refuted
Beta features 4.3× less adopted than GA despite identical usage intensity and error rates
Beta adoption grew 0% → 16.6% over 24 months — growing but still minority
...
---
Hypothesis Testing — All 28 Hypotheses
The project tested 28 explicit hypotheses across 8 analytical steps. Results:
...
Revenue growing while the business hollows out — MRR 2,291× alongside 20.17% churn simultaneously
No KPI has ever reached benchmark in 24 months — this is structural, not a recent deterioration
Churn is an acquisition quality problem — ads (60.2% churn) declining, partner (75.3%) growing
Enterprise concentration is existential — 98.2% MRR, 28.1% from 47 accounts
Month 1 is the highest-leverage intervention window — 36.4% of all churns happen in months 0–1
The retention matrix improvement is an illusion — the survival curve tells the true story
No feature has critical mass — zero switching cost, 5.05% max vs 20% benchmark
Support is structural deficit, not causal — escalation is the only confirmed signal (+6.5pp)
Risk model identifies $2.68M MRR intervention cohort — not a predictive engine
Three beta features are hidden retention anchors at <3.5% adoption
...
```
Validation
Every SQL file ends with a validation block. After running all layers, run the cross-validation check:
```sql
-- Spot-check: total accounts should be 500 everywhere
...
Reactivated accounts	326	✅ 326 (79.3% of 352 churned)
...
Dataset	Synthetic (Python-generated)
---
License
...
Questions about SQL architecture, analytical methodology, or findings? Open an issue.# ravenstack-saas-retention-analytics
